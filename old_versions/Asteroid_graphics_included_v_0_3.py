...
WHITE = (255,255,255)
GREEN = (0,255,0)
imp = pygame.image.load("vesmir_2.jpg")
...
class Asteroid:
    def __init__(self, x):
        ...
        self.mass = 1
        self.track = False
        self.image = pygame.image.load(random.choice(['asteroid_1.png','asteroid_2.png','asteroid_3.png']))
...
def draw_board(ship, asteroids, laser):
    ...
    for a in asteroids:
            pygame.draw.rect(DISPLAY_SURF, (GREY),
                            (a.asteroid_x_coord, a.asteroid_y_coord,
                            ASTEROID_SIZE*a.mass, ASTEROID_SIZE*a.mass))
            
            new = pygame.Rect(a.asteroid_x_coord, a.asteroid_y_coord, ASTEROID_SIZE*a.mass, ASTEROID_SIZE*a.mass)
            print(a.asteroid_x_coord, a.asteroid_y_coord)
            print(new.center)
...
#
def make_text(text, color, bg_color, top, left, Font):
...
def ruleta_screen():
    ...
    while True:
        ...
        red_text_surface = red_button_font.render('Red', True, BLACK)
        red_text_rect = red_text_surface.get_rect()
        red_text_rect.topleft = (55, WINDOWHEIGHT - 145)
        red_button_rect = pygame.Rect(45, WINDOWHEIGHT - 150, 90, 40)
        pygame.draw.rect(DISPLAY_SURF, RED, minus_100_button_rect)
        ...
        black_text_surface = black_button_font.render('Black', True, BLACK)
        black_text_rect = black_text_surface.get_rect()
        black_text_rect.topleft = (55, WINDOWHEIGHT - 145)
        black_button_rect = pygame.Rect(45, WINDOWHEIGHT - 150, 90, 40)
        pygame.draw.rect(DISPLAY_SURF, RED, minus_100_button_rect)
        ...
        green_text_surface = green_button_font.render('Green', True, BLACK)
        green_text_rect = green_text_surface.get_rect()
        green_text_rect.topleft = (55, WINDOWHEIGHT - 145)
        green_button_rect = pygame.Rect(45, WINDOWHEIGHT - 150, 90, 40)
        pygame.draw.rect(DISPLAY_SURF, RED, minus_100_button_rect)
    ...
    """
    result = spin()
    result_text_surface = BASIC_FONT.render(str(result), True, BLACK, WHITE)
    result_text_rect = result_text_surface.get_rect()
    result_text_rect.topleft = (WINDOWWIDTH//2, WINDOWHEIGHT//2 + 250)
    DISPLAY_SURF.blit(result_text_surface, result_text_rect)
    pygame.display.update()
    time.sleep(3)
    print("Ruleta result:", result)
    """
...
def load_json():
    with open('data_asteroid.json', 'r') as f:
        data = json.load(f)
        f.close()
    return data
...
def save_json(data):
    ...
    with open('data_asteroid.json', 'w') as f:
        f.write(data)
        f.close()
