...
def update_occuped_asteroid_spawn_positions():
    ...
    if len(asteroids) > ASTEROID_SIZE*2:
        asteroid = asteroids[ASTEROID_SIZE*2]
        if asteroid != None:
            x = asteroid.asteroid_x_coord
            size = asteroid.size
            occuped_asteroid_spawn_positions = [item for item in occuped_asteroid_spawn_positions if item not in list(range(x - 10, x + 10 + size))]
            print("uvolnene pozicie")
        asteroids = asteroids[:WINDOWHEIGHT+ASTEROID_SIZE]
...
def run_game():
    while True:
        asteroids.insert(1, spawn_asteroid())
        update_occuped_asteroid_spawn_positions()
        draw_board()
...
def draw_board():
    ...
    for asteroid in asteroids:
        if asteroid != None:
            asteroid_rect = pygame.Rect(asteroid.asteroid_x_coord, asteroids.index(asteroid) - ASTEROID_SIZE, asteroid.size, asteroid.size)
